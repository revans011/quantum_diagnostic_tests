...
#------------------------------------------------------------------
# 3. Data for the prostate DWI example
#    N = 300, y = 100
...
#      Sp    ~ Beta(12,8)
#      kappa ~ Beta(2,6)
#      eta   ~ Normal(0,0.3) truncated to [-1,1] (handled by bounds)
#------------------------------------------------------------------
stan_data <- list(
  N       = 300,
  y       = 100,
...
#------------------------------------------------------------------
# 5. Inspect posterior summaries
#------------------------------------------------------------------
fit$summary(variables = c("pi", "Se", "Sp", "kappa", "gamma", "eta", "p_pos"))

# Optionally extract draws as a data.frame
posterior_df <- as.data.frame(fit$draws(
  variables = c("pi", "Se", "Sp", "kappa", "gamma", "eta", "p_pos")
))
...
ggplot(posterior_df, aes(x = `1.pi`)) +
  geom_histogram(bins = 60, color = "white") +
  geom_vline(
    xintercept = median(posterior_df$pi),
    color = "red", linewidth = 1
  ) +
  theme_bw() +
  labs(title = "Posterior distribution of π",
       x = expression(pi),
       y = "Density")
```
Now do the classical version with no quantum part
