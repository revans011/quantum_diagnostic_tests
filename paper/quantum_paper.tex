...
\subsection{Quantum Bayes for a Single Binary Diagnostic Test}

Once a binary diagnostic test is written as a \(2\times 2\) density
matrix and updated using a measurement operator, the procedure is already
a form of quantum Bayesian inference.  
For a single binary test applied to a single latent class, the framework
is:

\begin{enumerate}
    \item treat the disease status as a two-level quantum system
          (a qubit),
    \item encode sensitivity and specificity in measurement operators
          acting on that system,
    \item apply the Lüders--Born update rule to the density matrix,
    \item read off posterior class probabilities from its diagonal
          entries.
\end{enumerate}

We present the construction step-by-step and show how the quantum update
reduces exactly to classical Bayes when no coherence is introduced.

\subsubsection*{1. Classical setup: one test, one latent class}
...
\subsubsection*{3. Encoding the diagnostic test as measurement operators}
...
\]
The normalized posterior state is
\[
\rho_D'(t)
=
\frac{\tilde{\rho}_D(t)}
...
Let \(\theta = (\pi, \mathrm{Se}, \mathrm{Sp})\) denote the unknown parameters.
Given data \(y\) positive results out of \(N\) tests, the likelihood is
\[
L(\theta \mid y)
= \binom{N}{y} \, p_+(\theta)^y \, [1 - p_+(\theta)]^{N-y},
\]
where
\[
p_+(\theta)
...
\[
\pi \sim \mathrm{Beta}(a_\pi, b_\pi), \qquad
\mathrm{Se} \sim \mathrm{Beta}(a_{\mathrm{Se}}, b_{\mathrm{Se}}), \qquad
\mathrm{Sp} \sim \mathrm{Beta}(a_{\mathrm{Sp}}, b_{\mathrm{Sp}}).
\]
