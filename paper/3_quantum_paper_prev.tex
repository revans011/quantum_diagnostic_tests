...
\subsubsection*{Embedding $D$ in a Hilbert space}
...
The prior distribution for $\pi$ becomes a density matrix.  
...
These satisfy completeness, \(M_0^\dagger M_0 + M_1^\dagger M_1 = I\), where The $\dagger$ symbol denotes the complex conjugate transpose.

\subsubsection*{Quantum Bayesian update (Lüders rule)}
...
\]
which is precisely Prof. Diggle's equation 2. We have therefore reproduced the biased estimate of prevalance from an imperfect diagnostic test in the discoherent state.
...
\label{eq:diggle_eq2}
\end{equation} which is exactly Prof. Diggle's equation 2 describing the bias in prevalence due to an imperfect diagnostic test. Thus, we have recovered the classical result using quantum notation.


\section{One imperfect test with quantum coherence, one population}

With the notation out of the way, and the classical formulas reproduced using quantum mechanics notation, we consider a single imperfect binary diagnostic test with coherence applied to one population.  
The latent disease status is classical; the quantum structure enters only through and measurement model for the test.

% ------------------------------------------------------------------

\subsection{Disease status}

Let \(D_i \in \{0,1\}\) denote the (unobserved) disease status for patient \(i=1,\dots,N\), and \(D_i = 1\) indicate the presence of disease, and \(D_i = 0\) indicate no disease.
...
In the quantum  representation, the effective ensemble state entering the diagnostic device is expressed as a \(2\times 2\) density matrix in the \(\{|0\rangle, |1\rangle\}\) basis corresponding to \(D=0\) and \(D=1\):
\[
\rho =
\begin{pmatrix}
1-\pi & c \\
...
\quad |\kappa| \le 1.
\]
The diagonal entries coincide with the classical prevalences, while the off-diagonal element \(c\) encodes the degree of phase coherence retained in the probe--sample interaction through a dimensionless parameter \(\kappa\). \(c^{*}\) is the complex conjugate of \(c\), and the constraint on \(\kappa\) ensures that \(\rho\) remains positive semidefinite, that is, ensures that all modeled probabilities lie between 0 and 1 and that no combination of parameters yields negative probability mass or impossible events.

The diagnostic test is modeled by a two-outcome positive operator-valued measure (POVM) \(\{E_+, E_-\}\), with
\[
E_+ =
\begin{pmatrix}
1-\mathrm{Sp} & \gamma \\
...
A POVM is the quantum analogue of a probabilistic classifier. Here, \(E_+\) corresponds to a positive test event.  
...
\label{eq:quantum_p_plus}
\end{align}

Recall that \(|\kappa| \le 1\) and \( |\gamma| \le \sqrt{(1-\mathrm{Sp})\,\mathrm{Se}}\), and note that \eqref{eq:quantum_p_plus} is \eqref{eq:diggle_eq2} with an additional bias term. Also, if either \(\kappa = 0\) (no coherence in the state) or \(\gamma = 0\) (measurement insensitive to coherence), then
\[
p_+ = (1-\mathrm{Sp})(1-\pi) + \mathrm{Se}\,\pi,
\]
the classical expression for a single diagnostic test, \eqref{eq:diggle_eq2}.

\subsection{Example diagnostic test and proposed priors}

In this section we demonstrate Bayesian inference for \(\pi\) using \eqref{eq:quantum_p_plus}. To illustrate reasonable prior choices for the parameters \((\pi, \mathrm{Se}, \mathrm{Sp}, \kappa,\) and \(\gamma)\), we consider a concrete medical imaging example: a binary diagnostic decision based on diffusion-weighted MRI (DWI) with ADC thresholding for detecting clinically significant prostate cancer in a referred cohort.  This modality provides high sensitivity but only moderate specificity in practice, and originates from coherent transverse spin magnetization, making it a natural example for a model that allows for limited coherence effects. Also, not the the priors were proposed by a large language model, an approach that is increasing in use. In a real data analysis, the priors should be vetted by domain experts; this example is for illustration only.

Let \(y\) denote the number of positive test results in a sample of size \(N\).  
Under the likelihood is then,
\[
y \mid N, p_+ \sim \mathrm{Binomial}(N, p_+),
\]

Our objective is Bayesian inference for \(\pi\).
...
These priors represent a synthesis of clinical parameters (for $\pi$,
$\mathrm{Se}$, and $\mathrm{Sp}$) and physical reasoning (for $\kappa$
and $\gamma$).  
They ensure that all quantum parameters remain within the
...
\subsection[The posterior distribution of pi]{The posterior distribution of \(\pi\)}

The posterior distribution for $\pi$ is obtained by combining the
likelihood with the priors on $\pi$, $\mathrm{Se}$, $\mathrm{Sp}$,
$\kappa$, and $\gamma$.  
Because the coherence term introduces the factor
$\sqrt{\pi(1-\pi)}$ into the likelihood, the resulting posterior does not
admit a closed-form expression.  
Posterior inference therefore requires numerical methods, such as Markov
chain Monte Carlo (MCMC).  

For illustration, we consider the case $N=300$ tests with $y=100$
positive results.  
Code for reproducing this example is available at:
\url{https://github.com/revans011/quantum_diagnostic_tests}.

% ------------------------------------------------------------------

\subsection{Identifiability: classical versus quantum models}
